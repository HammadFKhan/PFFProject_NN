...
from func import parametersL23_1
...
dt = 0.1        # time step (ms)
v_init = -73    # initial voltage (mV)
seed = int(time.time())        # random seed

P = parametersL23_1.init_params(wd)
np.random.seed(seed)

### create cell ##
cell = l5_neuron_model.L5Model(wd, P, verbool = True)
cell.set_rotation(z=-2*np.pi*((60. + 90.)/360.), rotation_order='zyx')
cell.set_pos(x = 500., y = 600.)
### electrode ###
x_off = -20
y_off = 0
z_off = 0
...
y = np.linspace(cell.yrange_entire[0]-850, cell.yrange_entire[1], nChannel) + y_off
y = np.flip(y, axis=0)
...
t, v = cell.simulate(T, dt, v_init, S_e, S_i)
visualization.plot_Vm_traces(cell, ['soma[0]', 'apic[10]'], [0,T], if_plot = 1)
