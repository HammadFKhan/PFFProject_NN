...
: added channel noise component

NEURON	{
	SUFFIX NaTs2_t_2F
	USEION na READ ena WRITE ina
	RANGE gNaTs2_tbar, gNaTs2_t, ina, NNaTs2_t
}
...
PROCEDURE rates(){
  ...
  qt = 2.3^((34-21)/10)
    ...
    if(v == -32){
		...
		SDm = sqrt(fabs(mAlpha*(1-m)+mBeta*m)/(0.05*NNaTs2_t*3))
    ...
    if(v == -60){
      v = v + 0.0001
		...
		SDh = sqrt(fabs(hAlpha*(1-h)+hBeta*h)/(0.05*NNaTs2_t))
	UNITSON
}